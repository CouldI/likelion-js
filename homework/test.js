...
function sourFruits(obj) {
    let result = [];
    for (fruit in obj){
        if(obj[fruit].taste === "sour"){
            result.push(fruit);
        }
    }
    return result;
...
function averagePrice(obj) {
    let total = 0;
    let count = 0;
    for (fruit in obj.fruits){
        total += obj.fruits[fruit].price;
        count++;
    }
    return total/count;
}
...
let storeAll = [storeA, storeB, storeC];



function nearestStore(x, y){
    let distance = [];
    storeAll.forEach(function(store){
        let dist = Math.pow(x - store.location[0],2) + Math.pow(y - store.location[1],2);
        distance.push(dist);
    });

    let nearDistance = distance.indexOf(Math.min.apply(null, distance));
    return storeAll[nearDistance].name;    
}

console.log(nearestStore(2,3));
// 4. 3번에 확장해서 원하는 과일을 넣으면 해당 과일을 파는 가장 가까운 상점 알려주기


function fruitFinder(x, y, fruit){
    const fruitIsThere = storeAll.filter(function(store){
        return store.fruits.hasOwnProperty(fruit);
    });
    
    if(fruitIsThere == false ){
        return "해당 과일을 파는 곳이 없습니다.";
    }

    let distance = [];
    fruitIsThere.forEach(function(store){
        let dist = Math.pow(x - store.location[0],2) + Math.pow(y - store.location[1],2);
        distance.push(dist);
    });

    let nearDistance = distance.indexOf(Math.min.apply(null, distance));
    return fruitIsThere[nearDistance].name;    
}

// 5. 4번에서 원하는 과일을 넣을 수도 있고, 안넣을 수도 있게 만들기. 즉, 원하는 과일이 있는 경우는 그걸 고려해서 알려주고 원하는 과일이 없으면 그냥 가까운 곳 알려주기

function fruitFinder2(x, y, fruit){
    if(typeof fruit !== "undefined"){
        if(fruitFinder(x, y, fruit) == "해당 과일을 파는 곳이 없습니다."){
            return nearestStore(x, y);
        } else {
            return fruitFinder(x, y, fruit);
        }                
    } else {
        return nearestStore(x, y);
    }    
...
buyFruit = function (obj, store) {
    for (fruit in obj){
        if(!store.fruits[fruit]){
            console.log('해당 상점에는' + fruit + '을 팔지 않습니다');
            continue;
        }
        if (store.fruits[fruit].stock < obj[fruit]) {
            console.log('해당 상점에 충분한 재고가 없습니다');
            continue;
        }
        store.fruits[fruit].stock -= obj[fruit];
        console.log(fruit + '을 ' + obj[fruit] + '개 구매하였습니다.');
        console.log('남은 재고:: ' + store.fruits[fruit].stock + '개 남았습니다');
    }   
};

buyFruit(buyList,storeB);

// 7. 6번에서 만든 함수에서 총 구매한 가격 리턴해주기

totalPrice = function (obj, store) {
    let total = 0;
    for (fruit in obj){
        if(!store.fruits[fruit]){
            continue;
        ...
        total += obj[fruit]*store.fruits[fruit].price
    }
    return total;
};

console.log(totalPrice(buyList, storeB));
